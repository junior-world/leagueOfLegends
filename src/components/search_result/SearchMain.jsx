...
const SearchMain = (props) => {
    ...
    return (
        <Main>
            <Col>
                {matchLists && matchLists.map( (matchList) => (
                    <RiotContext.Provider key={matchList.timestamp}  value ={{spellSummury,runsSummury}}>
                        <MatchLists  matchList ={matchList} searchInfo={searchInfo} champSummury={champSummury} spellSummury={spellSummury} runsSummury={runsSummury} />
                    </RiotContext.Provider>
                ))}

                <Col>
                    {run ?  <Button onClick ={onClickHandler}>더 보기</Button> : <Button > 로딩,,,</Button> }
                </Col>
            </Col>
        </Main>
    );
